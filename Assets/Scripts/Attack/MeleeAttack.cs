...
public class MeleeAttack : BaseAttack
{
    ...
    [SerializeField] private Renderer mobRenderer; // Сюда назначить Renderer модели моба
    [SerializeField] private Color[] glowColors = new Color[2] { Color.red, Color.blue };
    ...
    {
        if (mobRenderer == null || glowColors.Length < 2)
            yield break;

        Material material = mobRenderer.material; // ВАЖНО: не sharedMaterial, чтобы не менять глобально

        // Активируем Emission, если выключен
        material.EnableKeyword("_EMISSION");

        // Выбор случайного цвета
        Color chosenColor = glowColors[Random.Range(0, glowColors.Length)];

        // Применяем цвет (можно усилить, если Bloom слабый: `chosenColor * intensity`)
        material.SetColor(EmissionColorID, chosenColor);

        yield return new WaitForSeconds(glowDuration);

        // Отключаем свечение (через чёрный цвет)
